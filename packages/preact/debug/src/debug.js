...
import { options, Component } from '../../src/index.js';
import {
	ELEMENT_NODE,
	DOCUMENT_NODE,
...
import { assign, isNaN } from './util.js';
...
export function initDebug() {
	...
	options.diffed = vnode => {
		...
		if (vnode.__c != null && vnode.__c.__H != null) {
			...
			const hooks = vnode.__c.__H.__;
			if (hooks) {
				for (let i = 0; i < hooks.length; i += 1) {
					const hook = hooks[i];
					if (hook.__H) {
						for (let j = 0; j < hook.__H.length; j++) {
							const arg = hook.__H[j];
							if (isNaN(arg)) {
